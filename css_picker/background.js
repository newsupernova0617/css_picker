  ...
  handleMessage(message, sender, sendResponse) {
    ...
    switch (type) {
      ...
      case "CLERK_AUTH_SUCCESS":
        ...
        console.log('🔐 BACKGROUND: Received CLERK_AUTH_SUCCESS message:', message.data);
        this.handleClerkAuthSuccess(message.data).then(result => {
          console.log('🔐 BACKGROUND: Auth success handled:', result);
          sendResponse(result);
        }).catch(error => {
          console.error('🔐 BACKGROUND: Auth success handling failed:', error);
          sendResponse({ success: false, error: error.message });
        });
        return true; // 비동기 응답을 위해 true 반환
