...
const clerkClient = (typeof window !== 'undefined' && window.clerkClient)
  ? window.clerkClient
  : {
      isLoaded: false,
      isSignedIn: false,
...
class ConsoleManager {
  ...
  createMessageElement(message) {
    ...
    copyBtn.addEventListener('click', async (event) => {
      ...
      const canUse = await this.checkFeatureAccess('console_monitoring');
      if (!canUse) {
        return;
      }
...
class SidePanel {
  ...
  // 유저 표시 함수
  showUser(user) {
    const normalized = this.normalizeUserProfile(user);
    if (!normalized) {
      this.applySignedOutState();
      return;
    }
    ...
    if (planManager && typeof planManager.setUserContext === 'function') {
      const hasPlanFromProfile = Boolean(normalized.plan);
      planManager
        .setUserContext(normalized)
        .then(() => {
          if (!hasPlanFromProfile && typeof planManager.refreshPlanAndNotify === 'function') {
            return planManager.refreshPlanAndNotify('profile');
          }
          return null;
        })
        .catch((error) => {
    ...
    // Show/hide email verification warning
    const emailVerificationWarning = document.getElementById('emailVerificationWarning');
    if (emailVerificationWarning) {
      emailVerificationWarning.style.display = normalized.emailVerified ? 'none' : 'block';
    }
  ...
  setupElements() {
    ...
    this.premiumSection = document.getElementById("premiumSection");
  ...
  handleMessage(message, sender, sendResponse) {
    ...
    if (message.action === 'color-sampled') {
      this.processColorSample(message.color, message.coordinates);
      sendResponse({ success: true });
      return true;
    }
  ...
  async navigateToFeature(featureName, options = {}) {
    ...
    if (premiumFeatureMap[featureName]) {
      const allowed = await this.checkFeatureAccess(premiumFeatureMap[featureName]);
      if (!allowed) {
        return;
      }
    }
  ...
  saveColorToStorage(color) {
    try {
      ...
      const alreadyExists = existingColors.some((stored) => stored.id === color.id || stored.hex === color.hex);
      if (alreadyExists) {
        return;
      }
      existingColors.push(color);
      this.persistColorsToStorage(existingColors);
    } catch (error) {
  ...
  persistColorsToStorage(colors) {
    try {
      localStorage.setItem('css-picker-colors', JSON.stringify(colors));
    } catch (error) {
      console.error('Failed to persist colors:', error);
    }
  }

  removeColorFromStorage(colorId) {
    try {
      const filtered = this.loadColorsFromStorage().filter((color) => color.id !== colorId);
      this.persistColorsToStorage(filtered);
    } catch (error) {
      console.error('Failed to remove color from storage:', error);
    }
  }
  ...
  clearColorStorage() {
    try {
      ...
      console.error('Failed to clear color storage:', error);
    }
  }
  ...
  // 저장된 색상들 로드
  loadSavedColors() {
    const savedColors = this.loadColorsFromStorage();
  ...
  showSelectedColorInfo(color) {
    ...
    // 색상 포맷 업데이트
    const formats = this.generateColorFormats(color);
  ...
  deleteSelectedColor() {
    ...
    if (confirm(`Are you sure you want to delete ${this.selectedColor.hex}?`)) {
      this.removeColorFromStorage(this.selectedColor.id);
  ...
  generateHarmonyColors(baseColor, harmonyType) {
    if (!baseColor || !baseColor.hsl || !baseColor.rgb) {
      return [];
    }

    const { h: baseHue, s: baseS, l: baseL } = baseColor.hsl;
    const alpha = baseColor.hsl?.a ?? baseColor.rgb?.a ?? 1;
    const harmonies = [];

    const clamp = (value, min, max) => Math.min(Math.max(value, min), max);
    const normalizeHue = (hue) => (hue % 360 + 360) % 360;

    const createColor = (hue, saturation, lightness, label) => {
      const normalizedHue = normalizeHue(hue);
      const hsl = {
        h: Math.round(normalizedHue),
        s: clamp(Math.round(saturation), 0, 100),
        l: clamp(Math.round(lightness), 0, 100),
        a: alpha,
      };
      const rgb = this.hslToRgb(hsl.h, hsl.s, hsl.l);
      const hex = this.rgbToHex(rgb.r, rgb.g, rgb.b);
      return {
        id: `harmony_${label}_${normalizedHue}_${Date.now()}`,
        hex,
        rgb: { ...rgb, a: alpha },
        hsl,
        timestamp: Date.now(),
    ...
    switch (harmonyType) {
      case 'complementary':
        harmonies.push(createColor(baseHue + 180, baseS, baseL, 'complementary'));
        break;
      case 'analogous':
        harmonies.push(createColor(baseHue - 30, baseS, baseL, 'analogous-left'));
        harmonies.push(createColor(baseHue + 30, baseS, baseL, 'analogous-right'));
        break;
      case 'triadic':
        harmonies.push(createColor(baseHue + 120, baseS, baseL, 'triadic-a'));
        harmonies.push(createColor(baseHue - 120, baseS, baseL, 'triadic-b'));
        break;
      case 'tetradic':
        harmonies.push(createColor(baseHue + 90, baseS, baseL, 'tetradic-a'));
        harmonies.push(createColor(baseHue + 180, baseS, baseL, 'tetradic-b'));
        harmonies.push(createColor(baseHue + 270, baseS, baseL, 'tetradic-c'));
        break;
      case 'monochromatic':
        harmonies.push(createColor(baseHue, baseS, clamp(baseL + 15, 0, 100), 'mono-light'));
        harmonies.push(createColor(baseHue, baseS, clamp(baseL - 15, 0, 100), 'mono-dark'));
        break;
      default:
        harmonies.push(createColor(baseHue, baseS, baseL, 'base'));
        break;
    }
  ...
  generateColorHarmony() {
    ...
    const harmonies = this.generateHarmonyColors(this.selectedColor, harmonyType);
  ...
  async handleSignIn() {
    try {
      ...
      const loginResult = await this.sendBackgroundMessage({ type: 'login' });
  ...
  async fetchProfile() {
    const response = await this.sendBackgroundMessage({ type: 'get_profile' });
  ...
  normalizeUserProfile(user) {
    if (!user) return null;
    const rawPlan = user.plan || user.status || 'free';
    const normalizedPlan =
      (planManager && typeof planManager._normalizePlanValue === 'function')
        ? planManager._normalizePlanValue(rawPlan)
        : String(rawPlan || 'free').toLowerCase();
    const firstName = user.firstName || user.given_name || '';
    const lastName = user.lastName || user.family_name || '';
    const fullName = [firstName, lastName].filter(Boolean).join(' ');
    return {
      uid: user.uid || user.id || null,
      name: user.displayName || user.name || fullName || 'User',
  ...
  async refreshAuthState() {
    try {
      const rawUser = await this.fetchProfile();
      if (rawUser) {
        this.showUser(rawUser);
      } else {
        this.applySignedOutState();
      }
    } catch (error) {
  ...
  applySignedOutState() {
    ...
    if (planManager && typeof planManager.setUserContext === 'function') {
      planManager.setUserContext(null);
    }

    // Hide email verification warning on sign out
    const emailVerificationWarning = document.getElementById('emailVerificationWarning');
    if (emailVerificationWarning) {
  ...
  updatePlanUI() {
    if (this.premiumSection) {
      const isPremium = planManager && planManager.currentPlan === 'premium';
      this.premiumSection.style.display = isPremium ? 'block' : 'none';
    }

    if (this.homeAuthPrompt) {
      this.homeAuthPrompt.style.display = this.isSignedIn ? 'none' : 'block';
    }
  }
  ...
  */
  ...
  updateUserInfo() {
    ...
    if (this.userEmail) {
      this.userEmail.textContent = this.currentUser.email || 'No email';
    }
    
    if (this.userAvatar) {
      // 이름의 첫 글자를 아바타로 사용
      const initials = this.getInitialsFromName(this.currentUser.name);
  ...
  async initializePlanManagement() {
    if (!planManager) {
      console.warn('PlanManager not available during initialization.');
      return;
    }
    ...
    try {
      ...
      if (this.currentUser) {
        await planManager.setUserContext(this.currentUser);
      } else {
        await planManager.setUserContext(null);
      }

      this.setupPlanManagerCallback();
      const shouldSyncFromBackend = !this.currentUser?.plan;
      if (shouldSyncFromBackend) {
        await planManager.refreshPlanAndNotify('initial-load');
      }
      this.updatePlanUI();
      await this.setupPremiumLocks();
    } catch (error) {
      console.error('Failed to initialize plan management:', error);
    }
  }
  ...
  async setupFeatureLock(elementId, featureName, displayName) {
    try {
      ...
      if (!window.planManager && typeof planManager === 'undefined') {
        ...
        this.addPremiumLockOverlay(element, featureName, displayName || 'Premium Feature');
        return;
      }
    ...
    if (!canUse.allowed) {
      ...
      this.addPremiumLockOverlay(element, featureName, displayName);
      return;
    }
    ...
    this.restorePremiumLockStyles(element);
  } catch (error) {
  ...
  // Premium 잠금 오버레이 추가
  addPremiumLockOverlay(element, featureName, displayName = 'Premium Feature') {
    const existingOverlay = element.querySelector('.premium-lock-overlay');
    if (existingOverlay) {
    ...
    element.dataset.originalPointerEvents = originalStyles.pointerEvents || '';
    element.dataset.originalOpacity = originalStyles.opacity || '';
    element.dataset.originalFilter = originalStyles.filter || '';
    ...
    element.style.filter = 'grayscale(0.1)';

    const lockFeature = featureName || displayName.toLowerCase().replace(/\s+/g, '_');
    ...
    overlay.setAttribute('aria-label', `${displayName} requires Premium plan`);
    
    // 오버레이 클릭 이벤트
    const openUpgradeModal = (e) => {
      e.preventDefault();
      e.stopPropagation();
      this.showUpgradeModal(lockFeature, displayName);
    };

    overlay.addEventListener('click', openUpgradeModal);
  ...
  showUpgradeModal(featureName, displayName = null) {
    ...
    try {
      ...
      if (this.upgradeMessage) {
        if (!displayName) {
          this.upgradeMessage.textContent = upgradeInfo.message;
        } else if (upgradeInfo.message) {
          this.upgradeMessage.textContent = `${displayName}: ${upgradeInfo.message}`;
        } else {
