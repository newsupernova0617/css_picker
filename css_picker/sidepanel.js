...
// 이 클래스는 사용자 로그인/로그아웃과 세션 관리를 담당합니다
class ClerkExtensionClient {
  constructor(config) {
    this.config = config;          // Clerk 설정 정보
    this.user = null;              // 현재 로그인한 사용자 정보
    this.isLoaded = false;         // 클라이언트가 초기화되었는지 여부
    this.isSignedIn = false;       // 로그인 상태 여부
    this.sessionToken = null;      // 인증 세션 토큰
    this.init();                   // 초기화 함수 호출
  }
  ...
  async init() {
    try {
      // Chrome 저장소에서 이전에 저장된 인증 정보를 불러옵니다
      const result = await chrome.storage.local.get(['clerk_session', 'clerk_user']);
      if (result.clerk_session && result.clerk_user) {
        this.sessionToken = result.clerk_session;  // 세션 토큰 복원
        this.user = result.clerk_user;             // 사용자 정보 복원
        const isValid = await this.checkExistingSession(); // 세션 유효성 확인
        this.isSignedIn = isValid;
      }
      this.setupMessageListener();  // 메시지 리스너 설정
      this.isLoaded = true;
      this.notifyListeners('loaded'); // 로드 완료 알림
    } catch (error) {
  ...
  async handleAuthSuccess(authData) {
    try {
      // 인증 데이터를 클라이언트에 저장
      this.user = authData.user;                    // 사용자 정보
      this.sessionToken = authData.sessionToken;    // 세션 토큰
      this.isSignedIn = true;                       // 로그인 상태 변경
      
      // Chrome 저장소에 인증 정보를 영구 저장 (브라우저 재시작 시에도 유지)
  ...
  // 로그인 함수 - 새 탭에서 로그인 페이지를 연다
  async signIn() {
    try {
      const extensionId = chrome.runtime.id; // 확장 프로그램 ID 가져오기
      // 로그인 URL에 확장 프로그램 정보를 추가하여 새 탭에서 열기
      const authUrl = `${this.config.landingPageUrl}?extension_auth=true&extension_id=${extensionId}`;
      chrome.tabs.create({ url: authUrl }); // 새 탭에서 로그인 페이지 열기
...
// sidepanel.js (클라이언트 초기화 이후에 추가)
chrome.storage.onChanged.addListener((changes) => {
  if (changes.clerk_user) {
    console.log("🔄 SIDEPANEL: clerk_user updated:", changes.clerk_user.newValue);
