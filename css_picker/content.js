...
class ElementHighlighter {
  ...
  highlightElement(element, color) {
    // 이전에 하이라이트된 요소가 있다면 먼저 제거합니다
    this.clearHighlight();
    
    // 현재 요소의 outline 스타일을 백업합니다
    this.originalOutline = element.style.outline;
    
    // 새로운 테두리 스타일을 적용합니다
    element.style.outline = `${this.highlightWidth} solid ${color}`;
    element.style.outlineOffset = '1px'; // 테두리가 요소에서 1픽셀 떨어지게 설정
  ...
  clearHighlight() {
    ...
    if (this.currentHighlighted) {
      // 원래 outline 스타일로 복원합니다
      this.currentHighlighted.style.outline = this.originalOutline;
      this.currentHighlighted.style.outlineOffset = ''; // 기본값으로 복원
